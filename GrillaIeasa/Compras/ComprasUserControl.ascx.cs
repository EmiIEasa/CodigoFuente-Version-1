    ...
    public partial class ComprasUserControl : UserControl
    {
        ...
        private void CargarTabla()
        {
            ...
            string QuerySTRRe = "<View>" +
                                "<Query><Where><Eq><FieldRef Name='Estado' /><Value Type='Text'>Rechazado</Value></Eq></Where></Query>" +
                            "<RowLimit>250</RowLimit></View>";
            ...
            SPQuery querySub = new SPQuery();
            string QuerySTRSub = "<View>" +
                                "<Query><Where><Eq><FieldRef Name='Estado' /><Value Type='Text'>Subsanado</Value></Eq></Where></Query>" +
                            "<RowLimit>250</RowLimit></View>";
            querySub.ViewXml = QuerySTRSub;
            SPListItemCollection ListaSubsanados = SPContext.Current.Web.Lists["AltaProvisoria"].GetItems(querySub);
            foreach (SPListItem Facturas in ListaSubsanados)
            {
                ltTablaSubsanados.Text += "<tr>" +
                                            "<td>" + "<a href='" + SPContext.Current.Web.Url + "/_layouts/15/Ieasa/AltaProvisoria.aspx?ID=" + Facturas.ID.ToString() + "' class='alert-link'>" + Facturas.ID.ToString() + "</a></td>" +
                                            "<td>" + (Facturas["RazonSocial"] != null && !string.IsNullOrEmpty(Facturas["RazonSocial"].ToString()) ? Facturas["RazonSocial"].ToString() : String.Empty) + "</td>" +
                                            "<td>" + (Facturas["NombreFantasia"] != null && !string.IsNullOrEmpty(Facturas["NombreFantasia"].ToString()) ? Facturas["NombreFantasia"].ToString() : String.Empty) + "</td>" +
                                            "<td>" + (Facturas["Personeria"] != null && !string.IsNullOrEmpty(Facturas["Personeria"].ToString()) ? Facturas["Personeria"].ToString() : String.Empty) + "</td>" +
            ...
            SPQuery querySusBloq = new SPQuery();
            string QuerySTRSusBloq = "<View>" +
                                "<Query><Where><Eq><FieldRef Name='Estado' /><Value Type='Text'>Suspendido/Bloqueado</Value></Eq></Where></Query>" +
                            "<RowLimit>250</RowLimit></View>";
            querySusBloq.ViewXml = QuerySTRSusBloq;
            SPListItemCollection ListaSusBloq = SPContext.Current.Web.Lists["AltaProvisoria"].GetItems(querySusBloq);
            foreach (SPListItem Facturas in ListaSusBloq)
            {
